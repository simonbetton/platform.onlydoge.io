...
function buildHeaders(apiToken?: string, includeJson = false): Record<string, string> {
  return {
    ...jsonHeaders(includeJson),
    ...apiTokenHeaders(apiToken),
  };
}

function jsonHeaders(includeJson: boolean): Record<string, string> {
  return includeJson ? { 'content-type': 'application/json' } : {};
}

function apiTokenHeaders(apiToken: string | undefined): Record<string, string> {
...
async function readJsonError(response: Response): Promise<string | null> {
  try {
    ...
    return jsonErrorMessage(payload.error);
  } catch {
    return null;
  }
}

function isJsonErrorMessage(value: unknown): value is string {
  return [typeof value === 'string', Boolean(value)].every(Boolean);
}

function jsonErrorMessage(value: unknown): string | null {
  if (!isJsonErrorMessage(value)) {
    return null;
  }
...
async function createBootstrapKey(baseUrl: string): Promise<CreatedApiKey> {
  ...
  if (response.status === 401) {
    throw new Error('API keys already exist. Re-run with --api-token <api-token>.');
  }

  await assertOkResponse(response, 'could not create API key');
...
async function assertOkResponse(response: Response, label: string): Promise<void> {
  if (!response.ok) {
    throw new Error(`${label}: ${await readErrorMessage(response)}`);
  }
}
...
function printCurlRunsheet(input: {
  ...
  console.log('Set ONLYDOGE_API_TOKEN to the API token shown above before running these commands.');
  console.log(
