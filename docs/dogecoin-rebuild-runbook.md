...
The script checkpoints progress in metadata and can resume safely with the same height range. By default it writes replacement rows into `core_utxo_creates_v1`, `utxo_outputs_current_v2`, and the legacy `utxo_outputs_v2` fallback table; the `utxo_outputs_current_by_address_v2` projection is updated by the current-UTXO materialized view. For the address UTXO API surface, use `--target current --skipLegacy`; that uses the separate `dogecoin_script_pub_key_backfill_current_n{networkId}` checkpoint. Use `--writeMode mutation` only if replacement rows are not acceptable for a specific maintenance run.
