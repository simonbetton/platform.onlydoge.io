...
export function buildInvestigationQueryHttp(
  service: InvestigationQueryService,
  ...
  return new Elysia()
    ...
    .use(
      new Elysia({ prefix: '/v1/info' }).get(
        '/',
        ({ query, request }) => service.info(resolveAuthenticatedApiKey(request), query.q),
        {
