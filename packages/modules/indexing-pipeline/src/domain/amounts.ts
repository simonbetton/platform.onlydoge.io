...
export function allocateProRataAmount(
  ...
  return buildProRataAllocations(target, inputAddresses, inputTotals, totalInput);
}

function canAllocateProRata(inputAddresses: string[], totalInput: bigint): boolean {
  return [totalInput > 0n, inputAddresses.length > 0].every(Boolean);
}

function buildProRataAllocations(
  target: bigint,
  inputAddresses: string[],
  inputTotals: Map<string, bigint>,
  totalInput: bigint,
  ...
  let allocated = 0n;
  return inputAddresses.map((address, index) => {
    const amount = proRataAllocationAmount(
      target,
      inputAddresses,
      inputTotals,
      totalInput,
...
function proRataAllocationAmount(
  target: bigint,
  inputAddresses: string[],
  inputTotals: Map<string, bigint>,
  totalInput: bigint,
  ...
  if (isLastInputAddress(inputAddresses, index)) {
    return target - allocated;
  }

  return (target * amountContribution(inputTotals, address)) / totalInput;
}

function isLastInputAddress(inputAddresses: string[], index: number): boolean {
  return index === inputAddresses.length - 1;
}
...
function assertDecimalFractionPrecision(
  ...
  if (hasExcessDecimalPrecision(fraction, decimals)) {
    throw new ValidationError(`invalid decimal amount: ${raw}`);
  }
}

function hasExcessDecimalPrecision(fraction: string, decimals: number): boolean {
  return [fraction.length > decimals, /[^0]/u.test(fraction.slice(decimals))].every(Boolean);
}
