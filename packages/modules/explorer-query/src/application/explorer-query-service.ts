...
export class ExplorerQueryService {
  ...
  public async listMempool(
    ...
    const pageLimit = Math.min(limit ?? defaultMempoolLimit, maxMempoolLimit);
    const snapshot = await this.getCachedMempoolSnapshot(network);
    const transactions = snapshot.transactions.slice(pageOffset, pageOffset + pageLimit);
  ...
  private async getCachedMempoolSnapshot(
    ...
    const promise = this.loadMempoolSnapshot(network).catch((error) => {
      if (this.mempoolSnapshots.get(network.networkId)?.promise === promise) {
  ...
  private serializeMempoolTransaction(
    ...
    const sizeBytes = readNonNegativeInteger(entry.size) ?? readNonNegativeInteger(entry.vsize);
    const feeBase = readDecimalAmountBase(entry.fee ?? fees?.base);
    ...
    return {
      ...
      sizeBytes,
      feeBase,
      modifiedFeeBase: readDecimalAmountBase(entry.modifiedfee ?? fees?.modified),
      ...
      ancestorFeesBase: readDecimalAmountBase(entry.ancestorfees ?? fees?.ancestor),
      ...
      descendantFeesBase: readDecimalAmountBase(entry.descendantfees ?? fees?.descendant),
...
function compareMempoolTransactions(
  ...
  return (right.time ?? -1) - (left.time ?? -1) || left.txid.localeCompare(right.txid);
}
